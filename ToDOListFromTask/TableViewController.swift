...
class TableViewController: UITableViewController, UISearchBarDelegate {
    ...
    // Кнопка редактирования удалена вместе с функционалом
   ...
   /*
     // Неудачная попытка сделать так, чтобы completed ячейки красились в зеленый цвет
     
     
     func tableView(_ tableView: UITableView, indexPath: IndexPath){
        
        if model.changeState(at: indexPath.row) == true {
            tableView.cellForRow(at: indexPath)?.backgroundView?.backgroundColor = UIColor.green
        } else {
            tableView.cellForRow(at: indexPath)?.backgroundView?.backgroundColor = UIColor.white
    }
   }*/
